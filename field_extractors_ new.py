...
# mrp (PCR-R04)
# --------------------------------------------------------------------------
# Rewritten 2024 - the original version required an explicit "MRP"/"M.R.P."/
# "Maximum Retail Price" keyword directly adjacent to a recognised currency
# token (Rs./INR/₹). Two real-world failure modes slipped through:
#   1. Labels that say "PRICE 2500/-" or "RATE: 45/-" instead of "MRP" -
#      a very common Indian packaging convention, especially on cheaper
#      goods - never matched the keyword list at all.
#   2. Tesseract frequently mangles the ₹ glyph into stray punctuation, so
#      requiring an exact currency-token match right after the keyword
#      dropped otherwise-good matches whenever OCR botched just that one
#      symbol.
# Fix: widen the keyword list, allow up to 8 non-digit "junk" characters
# between the keyword and the amount (absorbs a garbled currency symbol
# without needing to recognise it), and detect currency from the whole
# line as a separate, more forgiving pass. Since PackSure targets the
# Legal Metrology Act (India), currency defaults to INR unless the line
# clearly says otherwise - this app is never going to see a genuine
# non-INR MRP on an Indian-market label.

_MRP_AMOUNT_PATTERN = re.compile(
    r"(?:mrp|m\.r\.p\.?|maximum\s*retail\s*price|selling\s*price|price|rate)"
    r"\s*[:\-]?\s*[^\d\n]{0,8}?"
    r"([\d]{1,6}(?:,[\d]{3})*(?:\.\d{1,2})?)"
    r"\s*(?:/-|/\s*-|\s*only)?",
    re.IGNORECASE,
...
_BARE_CURRENCY_PRICE_PATTERN = re.compile(
    r"(?:rs\.?|inr|₹)\s*([\d]{1,6}(?:,[\d]{3})*(?:\.\d{1,2})?)", re.IGNORECASE
)
...
def _detect_currency(text: str) -> str:
    if re.search(r"\$|usd", text, re.IGNORECASE):
        return "USD"
    return "INR"  # Rs./INR/₹, or no symbol at all -> assume INR for this market
...
def extract_mrp(lines: List[OCRLine]) -> dict:
    ...
    if len(bare_hits) > 1:
        ...
        # per the handoff doc, report the best candidate with
        # context_confirmed=False rather than guessing which one is MRP.
        i, line, amount = bare_hits[0]
        evidence = _base_evidence(line, amount.replace(",", ""), line.text, _context_window(lines, i))
        evidence["currency"] = _detect_currency(line.text)
